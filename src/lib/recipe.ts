...
import { formatVolumeExact, formatVolumeSmart, type VolumeUnit } from './units'
...
export function buildViewModel(
  ...
  let rows = colors
    ...
    .map(({ color, index, weight }) => {
      ...
      return {
        ...
        displayValue: unitMode === 'ml' ? formatVolumeSmart(exactMl, volumeUnit) : belowOnePercent ? '<1%' : `${percent}%`,
  ...
  const recipeItems: RecipeItem[] = rows.map((row, index) => ({
    ...
    percent: row.percent,
    recipeLine: `${formatVolumeSmart(row.exactMl, volumeUnit)} — ${row.displayName}`,
  }))
